...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      ...
      size: {
        default: "h-10 px-4 py-2 text-xs md:text-sm lg:text-base [&_svg]:size-3 md:[&_svg]:size-4",
        sm: "h-8 md:h-9 rounded-md px-2 md:px-3 text-xs md:text-sm [&_svg]:size-3",
        lg: "h-10 md:h-11 rounded-md px-6 md:px-8 text-sm md:text-base lg:text-lg [&_svg]:size-4 md:[&_svg]:size-5",
        icon: "h-8 w-8 md:h-10 md:w-10 [&_svg]:size-3 md:[&_svg]:size-4",
      },
    },
    defaultVariants: {
      variant: "default",
